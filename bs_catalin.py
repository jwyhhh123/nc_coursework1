...
class BackPropagation:
    ...
    def backward(self,x, y):
        ...
        #local gradients for softmax layer.

        i_k = 0
        for i_k in self.a[self.L-1]:
            if (i_k == np.argmax(y)):
                self.delta[self.L-1][i_k] = p - 1
            else:
                self.delta[self.L-1][i_k] = p
            i_k = i_k + 1

        #local gradients for hidden layers.

        layer_local = L - 2
        sum_k = 0

        while layer_local > 0:
            deriv_phi = self.phi_d[self.z[layer_local]]
            for j in range(len(self.z[layer_local])):
                for k in range(len(delta[layer_local])):
                    sum_k += self.delta[layer_local + 1][k] * self.w[layer_local+1][k][j]
            self.delta[layer_local][j] = deriv_phi * sum_k

            layer_local = layer_local - 1
